...
import { wordCategories, pickRandomWord, type WordData, type Difficulty } from "@/lib/words";
import { toast } from "sonner";
import { Bot, User, ArrowLeft, Loader2, Cpu } from "lucide-react";
...
import { pickAINames } from "@/lib/aiNames";
...
interface Player { id: string; name: string; isAI: boolean; isImposter: boolean; }
interface Hint { playerId: string; name: string; text: string; round: number; }
...
export default function AIMode() {
  ...
  const correct = vote && imposter && vote === imposter.id;

  useEffect(() => {
    if (phase !== "result" || confettiFired) return;
    setConfettiFired(true);
    if (correct) { fireConfetti(); setScore((s) => s + 1); }
  }, [phase, correct, confettiFired]);

  const start = () => {
    if (!playerName.trim()) return toast.error("Bitte gib deinen Namen ein");
    const picked = pickRandomWord(category, difficulty);
    ...
    all[Math.floor(Math.random() * all.length)].isImposter = true;
    setPlayers(all); setSecretWord(picked); setHints([]); setRound(1);
    setVote(null); setConfettiFired(false); setPhase("reveal");
  };

  const beginPlay = async () => { setPhase("playing"); await fetchAIHints(1); };
  ...
  const fetchAIHints = async (r: number) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("ai-imposter", {
        body: {
          category, word: secretWord.word, hint: secretWord.hint, round: r,
          previousHints: hints.map((h) => ({ name: h.name, text: h.text })),
          players: aiPlayers.map((p) => ({
            name: p.name, isImposter: p.isImposter,
            word: p.isImposter ? "" : secretWord.word, hint: secretWord.hint,
          })),
        },
      });
      ...
      console.error(e); toast.error(e instanceof Error ? e.message : "KI-Fehler");
  ...
  const nextRound = async () => {
    if (round >= 3) { setPhase("voting"); return; }
    const next = round + 1; setRound(next); await fetchAIHints(next);
  };

  const submitVote = (pid: string) => { setVote(pid); setPhase("result"); };

  const reset = () => {
    setPhase("setup"); setPlayers([]); setSecretWord(null); setHints([]);
    setRound(1); setVote(null); setConfettiFired(false);
    setMatchRound((r) => r + 1);
  };

  return (
    <div
      className="min-h-screen w-full px-5 py-8 md:px-10 md:py-12 page-enter"
      style={{
        background: "radial-gradient(ellipse at top, hsl(var(--game-accent) / 0.08), hsl(var(--game-bg-start)) 60%)",
    ...
    >
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          {score > 0 && (
            <div className="text-xs" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>
              Match {matchRound} · Score <span style={{ color: "hsl(var(--game-accent))" }}>{score}</span>
            </div>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "setup" && (
            <Card>
              <div className="p-4" style={{ background: "hsl(var(--game-input-bg))", border: "1px solid hsl(var(--game-border))", borderRadius: 2 }}>
                <p className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] mb-2"
                  style={{ fontFamily: mono, color: "hsl(var(--game-accent))" }}>
                  <Cpu className="h-3 w-3" /> Wie funktioniert die KI
                </p>
                <p className="text-sm" style={{ fontFamily: rubik }}>
                  Du und {aiCount} KI-Spieler bekommen Rollen. Einer ist der Imposter — du oder eine KI.
                  Pro Runde gibt jeder einen kurzen Hinweis zum Wort. Crewmate-KIs spielen ehrlich,
                  Imposter-KIs bluffen. Am Ende tippst du, wer es war.
                </p>
              </div>

              <Field label="Dein Name">
                <Input value={playerName} onChange={(e) => setPlayerName(e.target.value)} maxLength={20} className="term-input" style={inputStyle} />
              </Field>
              <Field label="Anzahl KI-Spieler">
                <Stepper value={aiCount} onChange={setAiCount} min={2} max={5} />
              </Field>
              <Field label="Thema">
            ...
            <Card>
              ...
              <RoleFlipCard
                ...
                front={
                  me.isImposter ? (
                    <>
                      <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: mono, color: "hsl(var(--game-accent))" }}>IMPOSTER</h2>
                      <p style={{ fontFamily: rubik }}>Du kennst das Wort NICHT.</p>
                      <p className="mt-2 text-lg font-bold" style={{ fontFamily: mono }}>„{secretWord.hint}"</p>
                    </>
                  ) : (
                    <>
                      <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: mono }}>CREWMATE</h2>
                      <p style={{ fontFamily: rubik }}>Geheimes Wort:</p>
                      <p className="mt-2 text-3xl font-bold" style={{ fontFamily: mono, color: "hsl(var(--game-accent))" }}>{secretWord.word}</p>
                    </>
                  )
                }
              />
              <div className="mt-6"><PrimaryButton onClick={beginPlay}>Los geht's</PrimaryButton></div>
            </Card>
          )}
          ...
          {phase === "voting" && (
            <>
              <div className="mb-4">
                <Countdown seconds={30} resetKey="vote" />
              </div>
              <Card>
                <p className="text-xs uppercase tracking-widest mb-3" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>
          ...
          {phase === "result" && imposter && secretWord && (
            <Card>
              <p className="text-xs uppercase tracking-widest mb-2" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>Auflösung</p>
              <p style={{ fontFamily: rubik }} className="mb-2">Du hast getippt auf:</p>
              <p className="text-xl font-bold mb-4" style={{ fontFamily: mono }}>{players.find((p) => p.id === vote)?.name}</p>
              <p style={{ fontFamily: rubik }} className="mb-2">Der Imposter war:</p>
              <p className="text-2xl font-bold mb-4 animate-flip-in" style={{ fontFamily: mono, color: "hsl(var(--game-accent))" }}>
              ...
              <div className="p-4 mb-6 text-center text-lg font-bold uppercase"
                style={{
                  fontFamily: mono, borderRadius: 2,
                  background: correct ? "hsl(var(--game-accent) / 0.15)" : "hsl(0 60% 50% / 0.15)",
                  border: `1px solid ${correct ? "hsl(var(--game-accent))" : "hsl(0 60% 50%)"}`,
                  color: correct ? "hsl(var(--game-accent))" : "hsl(0 80% 70%)",
                }}>
                {me?.isImposter
                  ? correct ? "Du hast dich selbst enttarnt 😅" : "Du hast als Imposter überlebt! 🎭"
                  : correct ? "Richtig! Imposter enttarnt 🎯" : "Falsch! Imposter ist entkommen 💀"}
              </div>

              <div className="mb-4 text-center text-xs" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>
                Match {matchRound} · Gesamtpunkte: <span style={{ color: "hsl(var(--game-accent))" }}>{score}</span>
              </div>
...
function HintRow({ hint, isMe }: { hint: Hint; isMe: boolean }) {
  return (
    <div className="p-3 flex items-start gap-3 animate-fade-in"
      ...
      <div className="flex-1">
        <p className="text-[10px] uppercase tracking-widest" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>{hint.name}</p>
...
function MyHintInput({ onSubmit }: { onSubmit: (t: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs uppercase tracking-widest" style={{ fontFamily: mono, color: "hsl(var(--game-accent))" }}>Dein Hinweis</p>
      <Input value={v} onChange={(e) => setV(e.target.value)} placeholder="Kurzer Hinweis..."
        maxLength={60} className="term-input" style={inputStyle}
        onKeyDown={(e) => { if (e.key === "Enter" && v.trim()) { onSubmit(v.trim()); setV(""); } }} />
      <button onClick={() => { if (v.trim()) { onSubmit(v.trim()); setV(""); } }}
        className="btn-press w-full py-2 text-xs uppercase tracking-widest"
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <label className="block text-[10px] uppercase tracking-widest" style={{ fontFamily: mono, color: "hsl(var(--game-secondary))" }}>{label}</label>
      {children}
    </div>
  );
}
